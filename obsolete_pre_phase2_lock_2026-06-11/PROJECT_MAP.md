...
- `run_edit_sd3.py`: main SD3 RF editing CLI and currently active argument parser.
...
`experiments/support_v3_2026-06-02/` currently contains completed E2-A SD3-matched RF-native comparison artifacts and completed native-context rows for `rf_solver_edit` (RF-Solver-Edit / RF-Edit, FLUX), `reflex` (ReFlex, FLUX), and `fireflow` (FLUX). `stable_flow` remains adapter/matrix-pending. `ot_rf_otip` (OT-RF / OTIP-style) and `dvrf` (DVRF / Delta Velocity RF) now have downloaded repos, built envs, help smoke, and one generation smoke, but no strict fixed-mask task matrix yet. Broad cross-backbone language should remain explicitly contextual, not an algorithmic superiority claim.
