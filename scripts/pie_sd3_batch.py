...
# load SD3 pipe ONCE, cache, neutralize per-call offload re-hooks
_orig = run_edit_sd3.StableDiffusion3Pipeline.from_pretrained
_cache={}
def _fp(*a, **k):
    ...
    if key not in _cache:
        ...
        p.to("cuda")
        _cache[key]=p; print("[sd3-batch] pipe loaded once",flush=True)
    return _cache[key]
...
OUT=PROJ/os.environ.get("OUT","outputs/pie_sd3_batch_20260618"); SEED="10"
import time
done=0
for e in man[:LIMIT]:
    ...
    argv=["--image",str((PROJ/e["image"]).resolve()),"--source-prompt",e["source_prompt"],"--prompt",e["target_prompt"],
      ...
      "--edit-core-scale","1.35","--edit-subject-scale","0.35","--region-target-transport-scale","0.0","--region-target-outside-lock-scale","0.0",
      "--rec-stop-timestep","0.08","--beta-max","1.0","--velocity-conversion-mode","linear_path","--linear-path-t-min","0.05",
      "--object-mask-provider","attention_velocity","--grounding-method","none","--edit-operation","add_object","--relation","on_surface","--mask-layering-mode","object_contact",
    ...
    sys.argv=["run_edit_sd3.py"]+argv
    t0=time.time()
    try:
        run_edit_sd3.main(); done+=1; print("OK",e["key"],"%.1fs"%(time.time()-t0),flush=True)
    except Exception as ex:
        import traceback; traceback.print_exc(); print("FAILED",e["key"],repr(ex),flush=True)
print("ALLDONE sd3_batch",done,"/",min(LIMIT,len(man)),flush=True)
