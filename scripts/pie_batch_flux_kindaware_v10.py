...
FLUX_STEPS=os.environ.get("FLUX_STEPS","28")
FLUX_N_MAX=os.environ.get("FLUX_N_MAX","24")
...
def argv_for(e, od, P):
    kind=kind_of(e)
    t=e["target_prompt"]
    if kind == "material" and T5_PROMPT_PREFIX:
        prefix=T5_PROMPT_PREFIX.rstrip(".")
        if prefix.lower() not in t.lower():
            t=prefix+". "+t
    if kind == "material" and T5_POSE_PROMPT:
    ...
    if FLUX_OFFLOAD == "sequential":
        ...
        a+=["--model-offload"]
    if kind == "material" and T5_NEGATIVE_PROMPT:
    ...
    mask=local_mask(e)
    if kind == "material" and T5_EXPAND_EDIT_MASK:
    ...
    if _adapt:
        for _fl, _vl in json.loads(_adapt).get(kind, {}).items():
            set_arg(a,_fl,_vl)
    return a
