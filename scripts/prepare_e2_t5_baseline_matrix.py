...
OUT = ROOT / "outputs" / "e2_t5_baseline_matrix"
...
def normalized_metadata(row: dict[str, str], src: Path) -> dict:
    ...
    metadata.setdefault("image", row.get("source_image", ""))
    metadata.setdefault("source_prompt", row.get("source_prompt", ""))
    metadata.setdefault("target_prompt", row.get("target_prompt", ""))
...
def main() -> int:
    ...
    for row in rows:
        ...
        (dst / "metadata.json").write_text(json.dumps(normalized_metadata(row, src / "metadata.json"), indent=2) + "\n", encoding="utf-8")
    ...
    copied_path = EXP / "e2_t5_baseline_matrix_manifest.csv"
    ...
    missing_path = EXP / "e2_t5_baseline_matrix_missing.csv"
