...
OUT = PROJ / os.environ.get("OUT", "outputs/flowedit_baselines/reflex")
...
def _get_pipe(*args, **kwargs):
    if "pipe" not in _cache:
        ...
        print("[flowedit-reflex] pipe loaded once", flush=True)
    return _cache["pipe"]
...
for item in man:
    final = OUT / item["key"] / "reflex" / f"seed_{SEED}" / "result.png"
    if final.exists():
...
print("ALLDONE flowedit-reflex", done, "/", len(man), flush=True)
