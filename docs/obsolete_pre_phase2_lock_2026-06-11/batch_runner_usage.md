...
```text
5 strict Core-5 tasks x support_v3_controller_rmsgap x seed 10 = 5 outputs
```
...
```text
5 strict Core-5 tasks x 4 E1 methods x seed 10 = 20 outputs
```
...
## Preserve-Floor Compatibility Flag

Some older metric runners still pass `--preserve-floor-csv` when evaluating E2
baseline-only outputs:
...
The current `evaluate_paper_metrics.py` accepts this flag for compatibility,
but it does not add preserve-floor or excess-preserve-error columns. Treat
outside-mask/source preservation metrics as the active preserve readout unless
the preserve-floor calculation is reintroduced and audited in code.
...
- `SCOPE`: `strict`, `implemented`, or `old_server_evidence`; use `strict` for
  paper-facing Core-5 work. Removal runs separately through the E5 probe scope.
