...
- `run_edit_sd3.py`: CLI entrypoint and currently active argument parser.
- `edit_cli_args.py`: parked split-out parser draft; not imported by the current
  runner until the parser split is finished.
...
- `scripts/run_pretty_matrix.sh`: slim matrix executor used by the paper wrapper.
- `scripts/core6_tasks.sh`, `scripts/core6_methods.sh`: historical filenames for separated task and method configuration; active scope is strict Core-5 plus the E5 removal probe.
...
3. Keep exploratory task/method variants in `legacy/cleanup_20260603/scripts/run_pretty_matrix.full_legacy.sh`; active Core-5 + E5 config still lives in historically named `scripts/core6_tasks.sh` and `scripts/core6_methods.sh`.
...
5. Keep metrics/grids pointed at the strict Core-5 headline list in `experiments/support_v3_2026-06-02/`, and keep removal plus older T2/T5 probes in diagnostic/supplement-only language unless explicitly scoped.
