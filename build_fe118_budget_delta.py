...
BUD = PROJ / "experiments/flowedit118_budget_fixedmask_metrics_20260619/metrics.csv"
def load(p):
    d = defaultdict(lambda: defaultdict(list))
    for r in csv.DictReader(open(p, newline="", encoding="utf-8")):
        if r["task"] not in PROBE or r["method"] != "dece_rf_flux": continue
        for m in ("edit_score", "inside_l1", "bg_l1"):
            try: v = float(r.get(m, "") or 0.0)
            except ValueError: v = 0.0
            d[fam(r["task"])][m].append(v)
    return d
o = load(ORIG); b = load(BUD)
print("=== dece_rf_flux FLUX budget-loosen (add/decal): editScr & bgL1 orig -> budget ===")
for f in ["T1", "T2", "T3", "T4", "T5"]:
    ov = o[f]["edit_score"]; bv = b[f]["edit_score"]
    obg = o[f]["bg_l1"]; bbg = b[f]["bg_l1"]
    ...
    tag = "  <-- add/decal (overridden)" if f in ("T1","T2","T3") else "  (unchanged kind)"
    print("  %s  editScr %.4f -> %.4f (%+.4f)   bgL1 %.4f -> %.4f (%+.4f)%s" % (f, mo, mb, mb-mo, go, gb, gb-go, tag))
