...
Status note: this draft is a working prose scaffold. The experiment section has
been resynchronized to the strict Core-5 headline suite plus the E5 removal
boundary probe, with E2-A as the SD3-matched comparison and E2-B as
native-backbone context. The source of truth remains
`paper/wacv_experiment_design.md`, `paper/results.md`, and
...
- `pillow_same_color_cable_knit`: localized same-color material replacement.

`backpack_remove_toy_charm` is reported separately as the E5 exposed-object
removal boundary probe, not as a Table 1 headline row.
...
audit artifacts; the E2 SD3-matched rows are used for method-context comparison
with the Table 2a common-subset caveat. The defensible claim is an
edit-preserve control result under matched SD3 backbone and reasonable support,
not broad arbitrary editing or superiority over FLUX editors.
...
- replacement/removal remains boundary/extension evidence rather than part of
  the strict Core-5 headline claim; recolor is included only as the controlled
