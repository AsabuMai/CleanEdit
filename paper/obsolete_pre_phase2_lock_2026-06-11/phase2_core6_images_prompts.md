...
- Verification Status: historical plan; revised by the 2026-06-10 Core-5 + E5 scope update
- Version Label: phase2_task_manifest_v0_historical_core6_filename

## Purpose

Scope revision 2026-06-10: the strict E1 set is Core-5 (T1-T5); the removal
rows below are E5 boundary-probe candidates, not headline E1 rows (see
`paper/wacv_experiment_design.md`, Scope Revision).

This manifest prepares the historical Phase 2 expansion. Under the current
scope, use it as a candidate pool for Core-5 operation-family expansions and
E5 boundary probes, not as an active six-category headline benchmark:

```text
5 headline categories x 3 source examples x 4 internal methods x 3 seeds = 180 E1 outputs, plus separately scoped E5 removal probes
```
...
- Treat E5 removal rows as simple exposed-object removal, not occluded host completion.
...
| T5 | Localized same-color material replacement | `pillow_same_color_cable_knit` | `data/phase2_candidates/pexels_white_pillow_brown_sofa_6312089.jpg` | `add_decal` | `on_surface` | canonical T5-1; entire pillow surface to same-color white cable-knit |
| T5 | Localized same-color material replacement | `pillow_same_color_cable_knit_grey` | `data/pretty_free_candidates/pexels_plain_pillow_sofa_phase1.jpg` | `add_decal` | `on_surface` | T5-1-style grey source; entire dark-grey pillow surface to same-color dark-grey cable-knit |
| T5 | Localized same-color material replacement | `pillow_same_color_cable_knit_armchair` | `data/phase2_candidates/pexels_green_chair_white_pillow_6312055.jpg` | `add_decal` | `on_surface` | T5-1-style armchair source; entire white pillow surface to same-color white cable-knit |
| E5 | Simple exposed-object removal | `backpack_remove_toy_charm` | `data/pretty_free_candidates/unsplash_backpack_keychain_njwnKDUDKNM.jpg` | `remove_object` | `remove_source_object` | E5 boundary probe; exposed charm |
| E5 | Simple exposed-object removal | `backpack_remove_silver_keychain_phase2` | `data/phase2_candidates/pexels_backpack_keychain_35171863.jpg` | `remove_object` | `remove_source_object` | E5 candidate; dark but localized keychain; needs support-mask gate |
| E5 | Simple exposed-object removal | `bag_remove_decorative_tag_phase2` | `data/phase2_candidates/pexels_bag_decorative_keychain_30853733.jpg` | `remove_object` | `remove_source_object` | E5 candidate; visible decorative tag; simple exposed removal |
...
### T5-2 `pillow_same_color_cable_knit_grey`
...
DECAL_COLOR=128,128,128
# Recipe matches T5-1: model-driven, no final reference composite, final
# source-chroma projection scale 1.0.
```

### T5-3 `pillow_same_color_cable_knit_armchair`
...
```text
A photo of the same plain white throw pillow on the same green velvet armchair, with the entire pillow surface changed into same-color white chunky cable-knit fabric with thick braided knitted columns, while preserving the pillow shape, seams, shadows, chair, tiled wall, floor, and the rest of the scene.
```
...
Retired T5 pattern probes: `pillow_same_color_linen_panel`,
`pillow_same_color_terry_panel`, `pillow_vertical_fabric_strip`,
`white_pillow_blue_dots_phase2`, and `white_pillow_blue_cross_phase2` are no
longer formal T5 tasks. The linen/terry rows are old center-panel material
probes and must stay diagnostic.

### E5-1 `backpack_remove_toy_charm`
...
### E5-2 `backpack_remove_silver_keychain_phase2`
...
### E5-3 `bag_remove_decorative_tag_phase2`
