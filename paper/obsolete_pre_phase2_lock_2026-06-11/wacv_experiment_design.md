...
Use this hierarchy throughout the paper and supplement:
...
Table 1 is the main effect, Table 2 is baseline fairness, Table 3 is
breadth/robustness validation, and Figures 4-6 carry mechanism and boundary
evidence. Do not call Phase 2 "Table 2 breadth".
...
| edit magnitude | inside-mask L1 / RMSE | inside fixed edit mask | local change amount only, not target correctness |
| semantic edit | CLIP target-source delta | full or local crop | auxiliary for add/decal; weak for recolor/material/removal |
...
Inside-mask L1/RMSE must never be described as edit success. A damaged local
region can have high inside-mask error, so inside L1/RMSE is only a local
change or edit-pressure proxy. Target correctness must come from task-specific
metrics and visual audit.

T4 and T5 should not mainly rely on CLIP. T4 recolor should report object-mask
color distance, target hue/saturation or blue-ratio shift, and outside color
leakage. T5 same-color material replacement should report crop-level material
audit, texture or high-frequency evidence where available, artifact score, and
outside preservation. T5 is valuable because color is controlled, but automatic
metrics must be interpreted conservatively.
...
For Strict Core-5, macro-average over the 5 tasks.
For Phase 2, report both an overall macro-average over 15 tasks and per-family
macro-averages over the five operation families.
```

If space allows, report mean +/- standard error in the main tables and
bootstrap confidence intervals in supplement.

Required fixed-mask sentence for the main experimental setup:
...
Main paper should show only runnable native/context rows, typically one to
three. Planned or blocked rows should move to a supplement implementation-status
table so the main paper does not look like an unfinished baseline matrix.
...
```text
Supplement Table S1, or Table 2c if space allows: support-matched diagnostic
```
...
Do not reserve Figure 4 for E2. Figure 4 is the support geometry ablation.
E2 evidence should be primarily tabular: Table 2a for same-backbone SD3, Table
2b for native/context rows, and Supplement Table S1 for support-matched
diagnostics. If a qualitative E2 panel is needed, use an unnumbered table inset
or a supplement figure and label backbones in every method name.
...
OT-RF / OTIP and DVRF: repos/envs/help smoke plus one generation smoke complete
on a100-01; strict fixed-mask adapter/matrix not yet complete
```
...
| relation correctness | whether better support improves the intended spatial relation |
| boundary artifact | whether support boundaries create visible seams or damage |
| locality / leakage | whether changes remain inside the intended region |
...
E4 is complete for the original compact SD3 controller-ablation subset and now
has a Phase2 T1-T4 expansion used for Table 2a joining:
...
Purpose: turn the Core-5 sanity signal into breadth/robustness validation while
making the E2 fairness structure explicit. Phase 2 should not be called a
large-scale, comprehensive, or general image-editing benchmark. Its paper-facing
role is:
...
Phase 2 should not merely add more baseline names; it should add the missing
calibration, preservation-aware, and support-matched controls that make the
comparison hard to attack.
...
| Figure 5 | E4 controller / stress Pareto curve | prove tradeoff behavior |
| Figure 6 | E5 boundary and failure probes | mark scope boundary |
...
| Figure 4 support geometry ablation | about 12-18 |
...
Use one main E2 algorithmic table plus one compact native/context summary by
default. Planned or blocked native rows belong in supplement implementation
status, not in a large main-paper table.

| Table | Content | Job |
| --- | --- | --- |
| Table 1 | Strict Core-5 main effect | headline quantitative evidence |
| Table 2a | E2.2 same-backbone SD3 algorithm comparison | primary RF algorithmic evidence |
| Table 2b | runnable native preservation-aware RF / FLUX contextual summary | cross-backbone context without algorithmic win claims |
| Table 3 | Phase 2 15-task common-subset breadth validation | robustness across additional sources |
...
If space is tight, keep detailed E3/E4 per-task rows and full ablation grids in
Supplement Tables S2/S3. Do not use Table 3 for component ablation; Table 3 is
reserved for Phase 2 15-task breadth validation.
...
If FLUX is blocked, write the blocker into the supplement implementation-status
table and proceed with same-backbone SD3 E2.2. Do not replace FLUX calibration
with unrelated diffusion baselines.
...
If all remain blocked, do not fill the main paper with a large blocked-method
table. Put exact failure reasons in the supplement implementation-status table
and keep Table 2b to runnable native/context rows or a short main-text summary.
