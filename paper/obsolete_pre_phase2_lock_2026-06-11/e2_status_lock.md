...
| SMOKE-VERIFIED | repo, environment, entrypoint/help check, and one generation smoke are verified, but the strict task matrix is not complete | implementation-status/context only |
...
| DVRF / Delta Velocity RF | route in `DeltaRectifiedFlowSampling` at `567b28b` | SMOKE-VERIFIED | status/context only until strict adapter/matrix complete | `_baselines/logs/generation_smoke/new_baselines_699815.out` reports `dvrf_status=0` on `a100-01`; no invented comparison |
