# Historical Phase 1 Images And Prompts
...
Filename note: this file keeps the historical `core6` name, but the active
paper-facing headline scope is strict Core-5 plus a separate E5 removal probe.

T5 status note: this Phase 1 file preserves earlier T5 planning history. The
current canonical T5 for the paper-facing runner is
`pillow_same_color_cable_knit` (2026-06-10: entire pillow surface to same-color
white cable-knit, model-driven recipe with final source-chroma projection),
because T5 is scoped as localized same-color material replacement rather than a
surface strip/decal probe. The earlier `pillow_same_color_corduroy_panel` and
`pillow_vertical_fabric_strip` tasks are diagnostic candidates.
...
| T5 | Localized same-color material replacement | `pillow_same_color_cable_knit` | `data/pretty_free_candidates/pexels_plain_pillow_sofa_phase1.jpg` | canonical; seeds 10/11/12 human visual gate passed 2026-06-10 |
...
Historical fallback only: if T5 cannot be implemented quickly, use
`tshirt_star` only as a second T3 decal diagnostic and label Phase 1 as Core-5
+ T3 expansion, not a material-replacement benchmark.
```
...
## T5 Historical Surface-Strip Diagnostic

This section records the earlier blue-strip diagnostic. It is not the current
paper-facing T5. The current canonical T5 is `pillow_same_color_cable_knit`,
which tests localized same-color material replacement over the pillow surface.

Historical runner task:
...
Historical implementation interpretation:
