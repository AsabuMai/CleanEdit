...
```text
CleanEdit prioritizes non-edit-region preservation in localized RF editing while retaining measurable target-local edit signal.
```

中文表述：

```text
CleanEdit 的目标是局部编辑时尽量保护非编辑区，同时保留可测的目标局部编辑信号。
```
...
CleanEdit visual selections and merged them into `phase2_internal_bg_metrics.csv`.
...
- CleanEdit 的定位是局部编辑与非编辑区保持之间的折中，特别强调非编辑区保护。
...
| CleanEdit-SD3 | 45 | 0.0279 | 0.1468 | 0.9254 | 0.8842 | 0.0333 |
...
- CleanEdit-SD3 的 outside L1 最低，说明非编辑区变化最小。
- CleanEdit-SD3 的 source SSIM luma 最高，说明源图结构保持最好。
- CleanEdit-SD3 的 edit_score 为正，说明不是单纯 no-op。
...
结论：CleanEdit-SD3 最符合“保护非编辑区，同时保留局部编辑信号”的设计目标。
...
| Fixed CleanEdit-SD3 | 45 | 0.0285 | 0.1440 | 0.9251 | 0.8619 | 0.0382 |
| CleanEdit-SD3 | 45 | 0.0279 | 0.1468 | 0.9254 | 0.8842 | 0.0333 |

### Table 2a Interpretation

Table 2a 说明 CleanEdit 的优势不是最大编辑强度，而是 preservation-first balance：

- FlowEdit-SD3 和 SplitFlow-SD3 更偏编辑激进，但 non-edit preservation 更弱；Sam-Flow-SD3 是保留的最近外部 SD3 baseline。
- Fixed CleanEdit-SD3 与 CleanEdit-SD3 非常接近，说明 CleanEdit clean-estimate decomposition 本身是主要贡献。
- CleanEdit-SD3 相比 Fixed CleanEdit-SD3 preservation 略好，但不能强写 adaptive feedback 带来巨大独立提升。

推荐写法：

```text
The adaptive rmsgap path stabilizes the preservation-oriented CleanEdit behavior,
...
Native RF/FLUX rows provide external context. They show that preservation-aware native methods can keep moderate edit signal, but their non-edit-region preservation is weaker than CleanEdit-SD3 in the SD3 same-subset table. Because backbone and implementation context differ, this should be presented cautiously as context rather than direct SOTA ranking.
...
- T1 attached accessory and T3 surface decal show clear CleanEdit target-local edit signal with better preservation than direct target.
- T4 recolor is a strong example of preservation: CleanEdit and generic support have similar outside L1, but CleanEdit source SSIM is much higher.
...
```text
On a controlled Phase2 T1-T5 diagnostic set, CleanEdit improves non-edit-region
...
- Table 1: CleanEdit-SD3 has best outside L1 and source SSIM among internal methods.
- Table 2a: same-backbone context shows stronger preservation than edit-aggressive SD3 baselines.
- Main qualitative grid: visually shows direct-target drift and CleanEdit preservation behavior.
...
- CleanEdit is a broad general-purpose image editor.
- CleanEdit is universally better than every baseline under every metric.
- CleanEdit has the strongest edit amplitude.
...
```text
CleanEdit is designed primarily for non-edit-region preservation in localized RF
...
```text
CleanEdit 的核心目标是局部编辑中的非编辑区保护。在 Phase2 T1-T5 诊断集上，
...
```text
Source | RF recon | Direct | Generic | FlowEdit | SplitFlow | Sam-Flow-SD3 | FireFlow | RF-Solver | ReFlex | Sam-Flow-FLUX | Fixed CleanEdit | CleanEdit
```
