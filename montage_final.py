import json, os
from PIL import Image, ImageDraw, ImageFont
base = "outputs/phase2_flux_paper_final_20260618"
tasks = sorted([d for d in os.listdir(base) if os.path.isdir(os.path.join(base,d))])
cell = 320
rows = []
for t in tasks:
    rundir = os.path.join(base, t, "dece_rf_flux", "seed_10")
    res = os.path.join(rundir, "result.png")
    meta = os.path.join(rundir, "metadata.json")
    src = None
    if os.path.exists(meta):
        m = json.load(open(meta))
        for k in ("source_image","source","src_image","image"):
            if k in m and m[k]:
                src = m[k]; break
    imgs = []
    for p in (src, res):
        if p and os.path.exists(p):
            im = Image.open(p).convert("RGB").resize((cell,cell))
        else:
            im = Image.new("RGB",(cell,cell),(40,40,40))
        imgs.append(im)
    row = Image.new("RGB",(cell*2+10, cell+22),(255,255,255))
    row.paste(imgs[0],(0,22)); row.paste(imgs[1],(cell+10,22))
    d = ImageDraw.Draw(row)
    d.text((4,4), t, fill=(0,0,0))
    rows.append(row)
W = cell*2+10
H = sum(r.height for r in rows)
canvas = Image.new("RGB",(W,H),(255,255,255))
y=0
for r in rows:
    canvas.paste(r,(0,y)); y+=r.height
canvas.save("/tmp/final_montage.png")
print("saved", canvas.size, "tasks", len(tasks))
