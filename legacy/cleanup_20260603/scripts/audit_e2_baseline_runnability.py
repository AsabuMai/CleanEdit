...
def write_markdown(path: Path, rows: list[dict[str, str]], strict_tasks: list[str]) -> None:
    ...
    lines = [
        ...
        "Date: 2026-06-02",
        "",
        "Claim boundary: downloaded source, command-level smoke, and strict Core-6 output validation are separate states. A method enters the reduced RF comparison only after revised strict Core-6 generation is stable and is evaluated with the same human/metric gates as CleanEdit.",
        "",
