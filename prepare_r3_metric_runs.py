...
MAN = json.load(open(PROJ / "data/flowedit_compatible_118/manifest_probe2.json"))
OUT = PROJ / "outputs/fe118_r3_metric_runs"
SRC = {"dece_rf_sd3": ("outputs/fe118_r3_sd3", "support_v3_controller_rmsgap"),
       "dece_rf_flux": ("outputs/fe118_r3_flux", "dece_rf_flux")}
def lc(src, dst):
...
for item in MAN:
    ...
    for method, (base, sub) in SRC.items():
        ...
        (rd / "command.txt").write_text("r3 linked from " + str(src) + "\n", encoding="utf-8")
        counts[method] += 1
