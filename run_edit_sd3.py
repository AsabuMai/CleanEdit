...
from recolor_projection import (
    _spatial_low_pass as _low_pass_tensor,
    _tensor_unit_image_to_pil as _unit_tensor_to_pil,
    _yuv_to_rgb as _yuv_to_rgb_tensor,
)
...
def transfer_source_luma_detail_inside_mask(
    ...
    source_luma_detail = source_yuv[:, :1] - _low_pass_tensor(source_yuv[:, :1], kernel_size)
    transferred_y = (result_yuv[:, :1] + float(strength) * source_luma_detail).clamp(0.0, 1.0)
    transferred_rgb = _yuv_to_rgb_tensor(torch.cat([transferred_y, result_yuv[:, 1:]], dim=1)).clamp(0.0, 1.0)
    return _unit_tensor_to_pil((result_rgb * (1.0 - mask) + transferred_rgb * mask).clamp(0.0, 1.0))
...
def main() -> None:
    ...
    x_tar = HRecSD3Edit(
        ...
        adaptive_component_control=args.adaptive_component_control,
        ...
        support_mask_policy=args.mask_policy,
        ...
        shared_core_shadow=args.shared_core_shadow,
        shared_core_authoritative=args.shared_core_authoritative,
    ...
    texture_detail_transfer_mask_path = None
    if args.edit_color_texture_detail_transfer:
        candidate_mask_paths = []
        if args.edit_color_texture_detail_transfer_mask:
            candidate_mask_paths.append(args.edit_color_texture_detail_transfer_mask)
        ...
        for candidate in candidate_mask_paths:
            if candidate and os.path.exists(candidate):
                texture_detail_transfer_mask_path = candidate
                break
        if texture_detail_transfer_mask_path is None:
            raise ValueError("--edit-color-texture-detail-transfer requires a color/edit mask")
        image_tar[0] = transfer_source_luma_detail_inside_mask(
            result_image=image_tar[0],
            source_image=PIL.Image.open(args.image).convert("RGB"),
            mask_image=PIL.Image.open(texture_detail_transfer_mask_path).convert("L"),
            strength=args.edit_color_texture_detail_transfer_strength,
            kernel_size=args.edit_color_texture_detail_transfer_kernel_size,
        )
    ...
    metadata = {
        ...
        "edit_color_texture_preserve_scale": args.edit_color_texture_preserve_scale,
        "edit_color_texture_kernel_size": args.edit_color_texture_kernel_size,
        "edit_color_texture_detail_transfer": args.edit_color_texture_detail_transfer,
        "edit_color_texture_detail_transfer_mask": texture_detail_transfer_mask_path,
        "edit_color_texture_detail_transfer_strength": args.edit_color_texture_detail_transfer_strength,
        "edit_color_texture_detail_transfer_kernel_size": args.edit_color_texture_detail_transfer_kernel_size,
        "edit_color_boundary_chroma_scale": args.edit_color_boundary_chroma_scale,
        "edit_color_boundary_kernel_size": args.edit_color_boundary_kernel_size,
        "edit_color_clean_projection_scale": args.edit_color_clean_projection_scale,
        "edit_color_clean_projection_mode": args.edit_color_clean_projection_mode,
    ...
    if args.shared_core_shadow:
        metadata["shared_core_shadow"] = True
    if args.shared_core_authoritative:
        metadata["shared_core_authoritative"] = True
